...
def test_live_cli_check_psm2010_suite_composition_uses_product_support() -> None:
    command = r'''
$env:POWERS_TOOL_LIVE_CLI_CHECK_IMPORT_ONLY = "1"
. .\scripts\live-cli-check.ps1
...
$script:PrivateArtifactDir = Join-Path (Get-Location) ".tmp_tests\psm-product-composition"
...
'''
    ...
    assert result.returncode == 0, result.stdout + result.stderr
    composition = json.loads(result.stdout)
    assert "gw-instek-psm-2010" not in composition["inventory"]
...
def test_live_cli_check_psm2010_full_plan_only_covers_product_commands() -> None:
    result = _run_live_cli_check(
        "-Target",
        "gw-instek-psm-2010",
        "-Connection",
    ...
    assert "regression validation of an already Product-open exact scope" in summary
    ...
    assert not any(
        "--validation-allow-pending-live-support" in case["arguments"]
