...
- Require exact integer `schema_version: 2` wherever the runtime contract
  ...
  `POST /command` responses, `files`-mode `request.json` and terminal
  `result.json`, memory-mode terminal results, and other schema-2 CLI JSON/JSONL
  envelopes. Reject missing versions, schema 1, strings, booleans, unsupported
  integers, fallback, or negotiation.
...
`append`, and caller-selected artifact paths. In `files` mode it writes fixed
job-local telemetry artifacts; in `memory` mode it creates no telemetry files
and streams schema-2 `sample` events on stdout. Core owns telemetry admission,
identity/support/channel validation, instrument reads, complete-cycle cadence,
and cooperative cancellation. CLI and Worker own their adapter-side CSV/JSONL
serialization; CLI owns `append` behavior, while Worker owns its
artifact-mode-specific telemetry evidence. Sequence `log` is a host-side
message/note action, not telemetry collection. A started telemetry
...
6. Treat HTTP `202` and `status: "accepted"` only as queue acceptance. Record
   the echoed `job_id` and generated `worker_job_id`; record `artifact_path`
   only when `files` mode provides it.
7. In `files` mode, verify `request.json` immediately exists after acceptance,
   then follow status and the accepted artifact directory until terminal
   `result.json` exists. Pending absence of `result.json` is not success.
8. In `memory` mode, expect no job directory, `request.json`, or `result.json`.
   Follow the terminal stdout JSONL event and/or `GET /status`
   `last_job.result`; missing filesystem artifacts are not failure.
9. Accept terminal success only when the mode-appropriate result has schema 2,
   status `succeeded`, `ok: true`, and matching `run_id` and `worker_job_id`.
   Treat `failed` and `cancelled` as non-success.
...
- `run_id` identifies one Worker runtime session and correlates stdout events,
  status responses, mode-appropriate terminal results, and final summary.
- `job_id` is the optional orchestrator-provided ID echoed by runtime
  ...
  into Power `files`-mode `request.json` or `result.json`.
- `worker_job_id` is generated by the Worker, identifies the job and, in
  `files` mode, its artifact directory, and is required for exact workflow
  cancellation.
...
`files`-mode E36312A simulator `read-status` smoke. It starts an owned simulate
Worker using the default `files` artifact mode, checks lifecycle and job
artifacts, cooperatively stops it, and produces a wrapper report. It rejects
live/arbitrary resources, alternate commands, and mode selection. Its
`request.json` and `result.json` checks are helper-specific, not a general
Worker requirement.
